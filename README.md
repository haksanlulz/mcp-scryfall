...
Counts as of 2026-09-14: 611 lines of server source (`wc -l index.ts server.ts`; `smoke.ts` is 277 more and is the live tier, and `scripts/bundle.mjs` 197 more and is build tooling, neither of them app code), 1,506 lines of tests (`wc -l test/*.ts`), 77 tests across 5 files — the number `npm test` reports. `grep -c "^\s*it(" test/*.ts` sums to 69, because two cases are table-driven: a `for` loop over three values, and an `it.each` over six that the pattern does not match at all. 69 + 2 + 6 = 77. Trust the runner.
...
Mutation probes, re-run 2026-09-14 against the 77-test suite. Three on the retry layer, because those cases do not share one kill — a `null` return, an unguarded parse and a missing floor break different parts of it:
