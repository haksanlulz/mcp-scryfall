...
| `card_search` | [Scryfall query-syntax](https://scryfall.com/docs/syntax) search. Returns compact summaries by default (pass `full: true` for raw objects). A query matching no cards is an error, not an empty list, because Scryfall answers a zero-result search with a 404. |
...
| `card_rulings` | Official Wizards rulings for one card, by exact name or Scryfall id. The errata and corner-case answers that aren't in the oracle text. |
